...
class PrevAction:
    ...
    def __init__(self, ch1, ra1, rn1, fd1,
        ...
        self.oppMaxAmt = max(ra1, ra2)
        self.maxBetRatio = discretizeOdds(self.oppMaxAmt, sps)
    def __hash__(self):
        #return int(1e4 * self.opp1CheckCallNum + 1e3 * self.opp1Aggr + 1e2 * self.opp2CheckCallNum + 1e1 * self.opp2Aggr + 1e0 * self.oppMaxAmt)
        return int(1e2 * self.opp1Aggr + 1e1 * self.opp2Aggr + 1e0 * self.maxBetRatio)
...
def discretizeOdds(bet_amt, pot):
    odds = float(bet_amt)/pot
    interval = [.6, 1.2, 2]
    return sum(i < odds for i in interval)
...
def discretizeEq(equity, numPlayers, street):
    ...
    if (numPlayers == 2):
        interval = [interval2[0]+0.01*street,
                    interval2[1]+0.03*street]
    else:
        interval = [interval3[0]+0.02*street,
                    interval3[1]+0.06*street]
